...
use super::settings::setters::{
    ...
    set_hunk_tracker_mode, set_invert_scroll, set_keep_text_selection, set_language,
...
pub(crate) fn dispatch(action: Action, app: &mut AppView) -> Vec<Effect> {
    ...
    let effects = match action {
        ...
        Action::Steer(text) => super::steer::dispatch_steer(app, text),
        Action::RollbackLast { receipt_id } => {
            super::steer::dispatch_rollback_last(app, receipt_id)
        }
        ...
        Action::SetLanguage(s) => set_language(app, s),
