...
/// Create a session for `agent_id` without routing through the project
/// picker. Restored after the 0.2.119 sync: upstream removed its own picker
/// and this helper with it, but `dispatch::live`'s unbound cold-start path
/// still needs "bind a session to THIS agent" (as opposed to
/// `Action::NewSession`, which allocates a different `AgentId`).
///
/// `mark_project_picker_done` is retained rather than dropped: turbo's picker
/// state still exists on `AppView`, so if the picker is ever re-wired this
/// path must continue to suppress it for a live cold start.
